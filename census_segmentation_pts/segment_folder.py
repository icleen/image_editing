...
def get_img(image_path):
    ...
    return full_res_img, org_img

def get_corners(org_img, sol):
...
if __name__ == '__main__':
    ...
    for f in os.listdir(image_folder):
        if ".jpg" in f or ".png" in f:
            big_img, sml_img = get_img(os.path.join(image_folder, f))
            corners = get_corners(sml_img, sol)
            pages[i] = write_segments(corners, big_img, f)
            i += 1
