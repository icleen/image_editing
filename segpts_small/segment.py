...
for i, c in enumerate(vert_clusters):
    ...
    # final_prediction_idxs = []
    # none_preditions = []
    for j, a in d.iteritems():
        output_grid[i,j] = predictions[a[1]]
        # final_prediction_idxs.append(a[1])

    # final_predictions = predictions[final_prediction_idxs]
...
for i,j in zip(*nan_idxs):
    ...
    print "i: {}, j: {}, output_grid[i, j]: {}".format(i, j, output_grid[i, j])

    i_neg = i-1
    print output_grid[i_neg, j, 0]
    ...
    j_neg = j-1
    while j_neg > -1 and np.isnan(output_grid[i, j_neg, 1]):
        j_neg -= 1
    j_neg = None if j_neg == -1 else j_neg

    j_pos = j+1
    while j_pos < output_grid.shape[1] and np.isnan(output_grid[i, j_pos, 1]):
        j_pos += 1
